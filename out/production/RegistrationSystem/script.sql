...
SELECT r.RegId,s.SId,s.SName,c.CId,c.CName,r.RegDate FROM Register r INNER JOIN Student s ON r. student_SId=s.SId INNER JOIN Course c ON r. course_CId=c.CId WHERE CId LIKE '%" + s + "%' or CName LIKE '%" + s + "%'